...
class Kueri {
  ...
  kue_nomor_otomatis(field,tabel) {
    let sql;
    if ( (field) && (tabel) ) {
      sql = 'SELECT (MAX(' + field + ') + 1) AS id FROM ' + tabel;
    }
    else{
      sql = "";
    }
...
module.exports = {
  ...
  NomorOtomatis: function (field,tabel) {
    return kuerinya.kue_nomor_otomatis(field,tabel);
  },
  ...
  Inputtan : function (str) {
    return fungsinya.inputtan(str);
  }
};